...
patches-own [ elevation lat long available  distRoad distHF numneighb]
...
to patches-availability ;; This command uses the count of shelters, not the count of refugees.
  ask patches [
    ...
    [ifelse elev-dif > 0.4 [
        set available (available * 0.65)]
      [ if elev-dif > 0.2 [
        set available (available * 0.8)]
    ]]   ]
  ...
  [
    ...
    if counter = 4 [
      ...
      if count-facilities / count-shelters <= ( 1 / initial-capacity ) [ ; =21.86 because of SPHERE standard: 1 health post per 10.000 people. (10.000 / 100 / hh size (4.57) )
        ...
        link-facility-shelters ]
      set counter 0
      set last-count count-shelters
    ]
  ]
    ;;;; to report:
  set average-distance (mean [travel] of links)
  set covered-shelters (count shelters with [covered? = True])
  set sumshelters (count shelters)
...
;    if p-sick <= 0.075714357 [ ; this is the avg amount of consults in 2019 divided by number of refugees.
    if p-sick <= 0.346344161 [ ; this is the avg amount of consults in 2019 divided by number of refugees multiplied by 4.57 (approx number of people in 1 hh).
;      use healthcare
      set sick? True
...
;  if ticks = 91 [set pop_max (1091 / 4.51434)] ; corresponds to June 2019
  ...
  if ticks = 10 [ set pop_max 180] ; corresponds to November 2017
  if ticks = 27 [ set pop_max 225] ; corresponds to March 2018
  if ticks = 35 [ set pop_max 222] ; corresponds to May 2018
  if ticks = 44 [ set pop_max 221] ; corresponds to July 2018
  if ticks = 53 [ set pop_max 236] ; corresponds to September 2018
  if ticks = 61 [ set pop_max 221] ; corresponds to November 2018
  if ticks = 65 [ set pop_max 241] ; corresponds to December 2018
  if ticks = 74 [ set pop_max 231] ; corresponds to February 2019
  if ticks = 91 [set pop_max 242] ; corresponds to June 2019
end
...
"# shelters" 1.0 0 -7500403 true "" "plot count shelters"
...
"" ""
PENS
"average-distance" 1.0 0 -16777216 true "" "plot average-distance"
"covered shelters" 1.0 0 -7500403 true "" "plot covered-shelters"
"total shelters" 1.0 0 -2674135 true "" "plot sumshelters"
"waiting patients" 1.0 0 -955883 true "" "plot sumwaiting-patients"
"capacity over-usage" 1.0 0 -6459832 true "" "plot sumovercapacity"
"total capacity" 1.0 0 -1184463 true "" "plot unusedcapacity"
